...
The web interface uses Python and Streamlit. The installed `groundmark` command also accepts a source file and output directory for terminal extraction through the same graph. `pypdfium2` renders PDF pages, Pillow handles raster images and annotations, and Pydantic validates the layout response. The page image takes precedence when preceding-page context disagrees with it.
...
Saved `ParseResult` JSON uses `schema_version: 2` and records `extraction_profile` as `legacy` or `detailed`. Older files load with unknown structure metadata and are not rewritten. `run_graph`, `parse_document`, and `parse_page` accept `detailed_layout=False`. The default request uses `LegacyParsePage` and the original prompt; detailed layout uses `ParsePage` and the structured prompt. Both become the same internal result type.

Detailed blocks add a nullable `structure` field with heading level, list items (text, marker, depth, checkbox state), and table-cell metadata (zero-based position, spans, header status). Cell text stays in the table array, and short rows get empty trailing cells. Validation rejects spans that overlap, exceed the table, leave gaps, or cover nonempty cells. It also rejects invalid list nesting. Every model-facing property is required; null indicates unavailable metadata.
...
`src/cli.py` selects UI startup or file extraction, loads endpoint settings, validates CLI inputs, and reports exit status. `run_graph` accepts optional `output_dir`, `formats`, and `view` arguments. Existing callers retain the full UI artifact set and per-run directories. CLI defaults select Markdown in the caller's output directory.

`src/export.py` writes selected formats from one `ParseResult`. The graph calls `parse_document` with `save_json=False` so JSON is saved only when requested; direct parser callers still get JSON by default. Figure extraction can return bytes without saving images. Annotation export can independently save PDF, page PNGs, and metadata. Individual export failures retain successful artifacts and are returned in `export_errors`. `output_paths` lists completed artifacts.

The Hatchling build includes the runtime package and maps authored `prompts/runtime/*.md` into package resources. The launcher uses an absolute installed app path and a writable user workspace. Wheels and source archives are distributed through GitHub Releases.
