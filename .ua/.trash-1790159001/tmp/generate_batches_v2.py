...
FILE_META = {
    ...
    "src/cli.py": {
        "summary": "Command-line interface providing subcommands for document parsing, layout extraction, and pipeline runs.",
        "tags": ["cli", "entry-point", "command-line", "runner"]
    },
    "src/export.py": {
        "summary": "Export serialization utilities converting extracted document artifacts to JSON, Markdown, and images.",
        "tags": ["export", "serialization", "utility", "formatting"]
    },
    "src/output_names.py": {
        "summary": "Standardizes output artifact naming conventions, page prefixes, and directory structures.",
        "tags": ["utility", "naming", "path-management"]
    },
    "src/chat.py": {
    ...
    "tests/test_cli.py": {
        "summary": "Tests CLI option parsing, help text, flag validations, and error exit codes.",
        "tags": ["test", "cli", "unit-test"]
    },
    "tests/test_cli_extraction.py": {
        "summary": "End-to-end integration tests for command-line document extraction and output generation.",
        "tags": ["test", "cli", "integration-test"]
    },
    ...
    "tests/test_output_names.py": {
        "summary": "Tests output filename generation, prefix sanitization, and path uniqueness rules.",
        "tags": ["test", "naming", "unit-test"]
    },
    "tests/test_parse.py": {
    ...
    "pyproject.toml": {
        "summary": "Modern Python project manifest configuring build metadata, dependencies, scripts, and tool settings.",
        "tags": ["configuration", "packaging", "build-system", "uv"]
    },
    "README.md": {
...
print("Synthesizing batch JSON files...")
...
for batch_idx in range(total_batches):
    ...
    for f in batch_files:
        ...
        if path.startswith("tests/test_"):
            base_name = path[len("tests/test_"):-3]
            candidates = [
                f"src/{base_name}.py",
                f"scripts/evaluate_{base_name}.py",
                f"src/ui/{base_name}.py"
            ]
            if base_name in ("layout_structure", "layout_evaluation"):
                candidates.extend(["src/layout.py", "scripts/evaluate_layout.py"])
                ...
                candidates.extend(["src/diagnostics.py", "src/ui/app.py"])
            elif base_name in ("cli", "cli_extraction"):
                candidates.append("src/cli.py")
            elif base_name == "output_names":
                candidates.append("src/output_names.py")
        ...
        if path == "README.md":
            for target in ["src/parse.py", "src/ui/app.py", "src/chat.py", "src/layout.py", "src/cli.py"]:
                edges.append({
            ...
            edges.append({"source": "config:pyproject.toml", "target": "file:src/__init__.py", "type": "configures", "direction": "forward", "weight": 0.6})
