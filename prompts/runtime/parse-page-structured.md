...
- `page_header` / `page_footer`: running page furniture, such as repeated fax transmission lines and page numbers. Do not classify a document title, substantive section, stamp, handwriting, or unique clinical/business information as page furniture. When uncertain, keep the original content type.
...
- Set `structure` to null when no structural information applies. Otherwise supply all three fields: `heading_level`, `list_items`, and `table_cells`; use null for fields unrelated to the block.
- For titles and headings, set `heading_level` to 1 through 6, preserving the visible parent/subsection hierarchy. Use 1 for a document title. Do not turn the remaining lines of a heading into body text.
- For lists, set `list_items` in reading order. Each item has `text` (the exact item text without its marker), `marker` (the visible bullet or numbering), `depth` (zero for outermost, increasing at most one level), and `checked` (true/false only for an actual checkbox, otherwise null). Keep wrapped lines in the same item; preserve the full original transcription in the block's `text`.
- For tables, set `table_cells` to metadata for each cell's top-left position, including empty cells. Each entry has zero-based `row` and `column`, positive `rowspan` and `colspan`, and boolean `is_header`. Mark headers only when visibly supported; a table may have no headers. Every grid position must be covered exactly once. Put merged-cell text only at its top-left position in `table`, leaving covered positions empty. Ordinary cells have spans of 1. Do not duplicate cell text in metadata or invent column labels.
- Use paragraph blocks for prose. Keep meaningful line breaks in addresses and other multiline fields; do not split a paragraph merely because the printed line wraps.
